...
// ✅ Get single station stats by stationCode
// router.get("/:stationCode", async (req, res) => {
//   const { stationCode } = req.params;

//   try {
//     const statDoc = await db.collection("station_stats").doc(stationCode).get();
//     if (!statDoc.exists) {
//       return res.status(404).json({ error: "Station stats not found ❌" });
//     }

//     const stats = statDoc.data();
//     const imei = stats.imei;

//     const baseDoc = imei ? await db.collection("stations").doc(imei).get() : null;
//     const base = baseDoc?.exists ? baseDoc.data() : {};
...
//       iccid: base.iccid || "",
//       location: base.location || "",
//       totalSlots: base.totalSlots || 6,
//       availableCount: stats.availableCount || 0,
//       rentedCount: stats.rentedCount || 0,
//       station_status: stats.station_status || "Offline",
...
//   try {
//     const statDoc = await db.collection("station_stats").doc(stationCode).get();
//     const baseDoc = await db.collection("stations").doc(stationCode).get();

//     // ❌ Haddii labaduba aysan jirin
//     if (!statDoc.exists && !baseDoc.exists) {
//       return res.status(404).json({ error: "Station not found ❌" });
//     }
...
//     // ✅ Default values haddii aan la helin station_stats
//     res.status(200).json({
//       id: stationCode,
//       stationCode,
//       imei: base.imei || stats.imei || "",
//       name: base.name || "",
//       iccid: base.iccid || "",
//       location: base.location || "",
//       totalSlots: base.totalSlots || 6,
