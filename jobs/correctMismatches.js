...
// Step 2: Correct mismatches for moved batteries
export async function correctMismatches() {
  ...
  for (const doc of rentalsSnap.docs) {
    ...
    // --- Only close rentals if battery is physically at another station ---
    const physical = liveMap[battery_id];
    if (physical && physical.stationId !== imei) {
      console.log(
        `👻 Closing old rental: battery ${battery_id} rented at ${imei}, found at ${physical.stationId}`
      );
      ...
      // Auto-close the old rental only
